 ...
 * GET     /(api/result/)              ->  index
 * POST    /(api/result/)              ->  create
 * GET     /(api/result/)/:id          ->  show
 * PUT     /(api/result/)/:id          ->  upsert
 * PATCH   /(api/result/)/:id          ->  patch
 * DELETE  /(api/result/)/:id          ->  destroy
 */
...
export function resultList(req, res) {
    res.send('This is result array')
}
...
export function upsert(req, res) {
    ...
    return Result.findOneAndUpdate({_id: req.params.id}, req.body, {new: true, upsert: true, setDefaultsOnInsert: true, runValidators: true}).exec()
        .then(respondWithResult(res))
        .catch(handleError(res));
}
